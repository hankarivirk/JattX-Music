...
Group settings panel: play mode, language, clean mode, auto-end toggle.
"""
...
def _settings_buttons(chat_id: int) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🌍 Language",   callback_data=f"set_lang_{chat_id}"),
            InlineKeyboardButton("▶️ Play Mode",  callback_data=f"set_mode_{chat_id}"),
        ],
        [
            InlineKeyboardButton("🧹 Clean Mode", callback_data=f"set_clean_{chat_id}"),
            InlineKeyboardButton("⏹ Auto End",   callback_data=f"set_autoend_{chat_id}"),
        ],
...
def _lang_buttons(chat_id: int) -> InlineKeyboardMarkup:
    ...
    for code, name in _LANGS.items():
        row.append(InlineKeyboardButton(name, callback_data=f"setlang_{code}_{chat_id}"))
        if len(row) == 2:
            rows.append(row); row = []
    if row:
        rows.append(row)
    rows.append([InlineKeyboardButton("« Back", callback_data=f"settings_{chat_id}")])
...
async def setlang_cb(client: Client, cb: CallbackQuery):
    ...
    await cb.answer(f"✅ Language set to {_LANGS.get(lang, lang)}", show_alert=True)
    await cb.message.edit_text(
        await _settings_text(chat_id),
        reply_markup=_settings_buttons(chat_id),
    )
...
async def set_clean_cb(client: Client, cb: CallbackQuery):
    ...
    await cb.answer(f"🧹 Clean Mode: {'On' if nxt else 'Off'}", show_alert=True)
    await cb.message.edit_text(
        await _settings_text(chat_id),
        reply_markup=_settings_buttons(chat_id),
    )
...
async def set_autoend_cb(client: Client, cb: CallbackQuery):
    ...
    await cb.answer(f"⏹ Auto End: {'On' if nxt else 'Off'}", show_alert=True)
    await cb.message.edit_text(
        await _settings_text(chat_id),
        reply_markup=_settings_buttons(chat_id),
    )
